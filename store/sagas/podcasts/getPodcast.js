...
async function getPodcastApi(slug) {
  const res = await fetch(`https://cryptic-activist-podcast-api.herokuapp.com/podcasts/get/slug/${slug}`, {
    method: 'GET',
    mode: 'cors',
    cache: 'no-cache',
...
async function getRelatedPodcastsApi(category, slug) {
  const res = await fetch(`https://cryptic-activist-podcast-api.herokuapp.com/podcasts/get/category/newest/${category}/${slug}`, {
    method: 'GET',
    mode: 'cors',
    cache: 'no-cache',
    credentials: 'same-origin',
